...
function getVisibleCount() {
    ...
    return currentCategoryPager === 'web' ? 4 : 6;
}
...
function renderPage(cat) {
    ...
    if (grid) grid.dataset.activeCategory = cat;
...
------------------------------------------------------------------ */
function initializeRcDigitalLegacy() {
    if (window.__rcLegacyInitialized) return;
    window.__rcLegacyInitialized = true;
    
    // -- Initialize Feather Icons & AOS --
    if (typeof feather !== 'undefined') feather.replace();

    // -- Lightweight native scroll reveals (React-safe, cold-cache safe) --
    ...
    const revealNowOrObserve = (el) => {
      if (!(el instanceof Element) || !el.matches('[data-aos]')) return;
      ...
      if (rect.top < window.innerHeight * 1.02 && rect.bottom > -40) {
        el.classList.add('aos-animate');
      } else {
    ...
    const bindRevealItems = (rootNode = document) => {
      if (rootNode instanceof Element && rootNode.matches('[data-aos]')) {
        revealNowOrObserve(rootNode);
      }
      if (rootNode.querySelectorAll) {
        rootNode.querySelectorAll('[data-aos]').forEach(revealNowOrObserve);
      }
    };

    // Bind anything already mounted.
    bindRevealItems(document);

    // React can mount the page after this legacy script has initialized.
    const revealMutationObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === 1) bindRevealItems(node);
        });
      });
    });

    revealMutationObserver.observe(document.documentElement, {
      childList: true,
      subtree: true
    });

    // Extra safety passes for cold/incognito startup timing.
    requestAnimationFrame(() => {
      bindRevealItems(document);
      requestAnimationFrame(() => bindRevealItems(document));
    });
    window.addEventListener('load', () => bindRevealItems(document), { once: true });
...
const adsData = [
    {
        ...
        img: "Advertisements/Digital Portfolio.webp",
        cta: "Request Support",
    ...
    },
    {
        ...
        img: "Advertisements/Chatbot Integration.webp",
        cta: "Request Quote",
        viewLabel: "View design",
    ...
    },
    {
        ...
        img: "Advertisements/Domain Registration & Hosting.webp",
        cta: "Request Invoice",
        viewLabel: "View design",
    ...
    },
    {
        ...
        img: "Advertisements/Standard Business Website.webp",
        cta: "Request Invoice",
        viewLabel: "View design",
    ...
    },
    {
        ...
        img: "Advertisements/business Starter Pack 1.webp",
        cta: "Request Invoice",
        viewLabel: "View design",
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeRcDigitalLegacy, { once: true });
} else {
  // React may still be mounting; wait for bridge event.
  window.addEventListener('rc-react-mounted', initializeRcDigitalLegacy, { once: true });
}
window.addEventListener('rc-react-mounted', initializeRcDigitalLegacy, { once: true });
